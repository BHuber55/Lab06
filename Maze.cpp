...
Cell* Maze::processBackTrack(StackLinked<Cell>* stack)		//should be done.
{
	//DO THIS
...
bool Maze::isSolved(Cell* curr_cell, StackLinked<Cell>* stack)
{
	...
	if (row == height && col == width && maze->getElement(height, width) == 1)
	{
		//set the maze location to TRIED
		maze->setElement(height, width, 2);
...
bool Maze::traverse()
{
	...
	while (!stack.isEmpty())
	{
		...
		int col = curr_cell->getCol();therwise delete it
			...
			//check that the current maze location corresponds to SPACE, o
